%Function to compare Classification error with no. of samples
%Priors are defined
Prior1=0.5;
Prior2=0.5;
...
%For training use 500 points of 1 class and 500 points of another class
    E_SVM=zeros(samT,1);
    VAR_SVM=zeros(samT,1);
    E_P=zeros(samT,1);
    VAR_P=zeros(samT,1);
    E_MLE=zeros(samT,1);
    VAR_MLE=zeros(samT,1);
    E_LDA=zeros(samT,1);
    VAR_LDA=zeros(samT,1);
...
for sam=1:10 % As distance between 2 distributions are varied
    ...
    E_SVM(sam)=mean(e_svm);
    VAR_SVM(sam)=var(e_svm);
    E_P(sam)=mean(e_p);
    VAR_P(sam)=var(e_p);
    E_MLE(sam)=mean(e_mle);
    VAR_MLE(sam)=var(e_mle);
    E_LDA(sam)=mean(e_lda);
    VAR_LDA(sam)=var(e_lda);
end
%legend('Bayes');
%plot(Ttr);
%errorbar(Tte,TteVAR);
